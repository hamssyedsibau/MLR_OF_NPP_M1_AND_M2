...
## start linear regression (Model 1)
# adjust the time cycles for regression
nt = nt#2*365 # because data avalible day (replace this with nt to process at all avalible times in dataset)
T  = 365 # total number of days
t = 1:1:nt # time measure in days

# preallocate the matrix for R-squared values
M1_R2 = zeros(length(mlat),length(mlon))

for i in 1:length(mlat)
    for j in 1:length(mlon)
        # construct the predictive matrix A
        nh=3 # set no. of harmonics
        # 1 + 2*nh => mean (col. of ones) + 3 harmonics (cols. of cos and sin)
        A = Array{Union{Missing, Float32}}(missing,nt,1+2*nh)
        A[:,1] = ones(nt,1) # column of ones
        l = 1 # set counter for harmonics
        for m in 1:nh
        ...
        if  (idxA==true || idxY==true)
            ...
            b = inv(A'*A)*(A'*Y) # normal form
            #b = A\Y # compute the regression co-efficients
            YModel = A*b#A[:,1:3]*b[1:3]
            e = sum((Y - YModel).*(Y - YModel))  # variance
            M1_R2[i,j] = 1 - (e ./ sum(Y.*Y)) # compute r-squared
        end
    end
end


## start linear regression (Model 2)
# preallocate the matrix for R-squared values
M2_R2 = zeros(length(mlat),length(mlon))
# adjust the time cycles for regression
nt = nt#2*365 # because data avalible day (replace this with nt to process at all avalible times in dataset)
T  = 365 # total number of days
t = 1:1:nt # time measure in days
...
for i in 1:length(mlat)
    for j in 1:length(mlon)
        # construct the predictive matrix A
        nh=3 # set no. of harmonics
        # 1 + 2*nh + 2 => mean (col. of ones) +
        # 3 harmonics (cols. of cos and sin) +
        # DIV_00 (col. of div at z=0) + DIV_15 (col. of div at z=0)
        A = Array{Union{Missing, Float32}}(missing,nt,1+2*nh+2)
        A[:,1] = ones(nt,1) # column of ones
        l = 1 # set counter for harmonics
        for m in 1:nh
            A[:,l+1] = cos.(2*pi*m*t/T)
        ...
        A[:,l+1] = reshape(DIV_00[i,j,1:nt],nt,1) # divergence at z = 0 m
        A[:,l+2] = reshape(DIV_15[i,j,1:nt],nt,1) # divergence at z = 15 m
        ...
        if  (idxA==true || idxY==true)
            ...
            YModel = A*b#A[:,1:3]*b[1:3]
            e = sum((Y - YModel).*(Y - YModel))  # variance
            M2_R2[i,j] = 1 - (e ./ sum(Y.*Y)) # compute r-squared
        end
    end
end
...
min_M1_R2 = minimum(M1_R2)
max_M1_R2 = maximum(M1_R2)
min_M2_R2 = minimum(M2_R2)
max_M2_R2 = maximum(M2_R2)
minR2 = min(min_M1_R2,min_M2_R2)
maxR2 = max(max_M1_R2,max_M2_R2)
p1= contourf(xg,yg,M1_R2,title = "mean + 3 harmonics",linewidth=0, levels=minR2:0.08:maxR2,showlabels = true,size=(1300,1000),xtickfont=18,ytickfont=18)
p2= contourf(xg,yg,M2_R2,title = "mean + 3 harmonics + div0 + div15",linewidth=0, levels=minR2:0.08:maxR2,showlabels = true,size=(1300,1000),xtickfont=18,ytickfont=18)

minR2 = minimum(M2_R2-M1_R2)
maxR2 = maximum(M2_R2-M1_R2)
p3= contourf(xg,yg,M2_R2-M1_R2,title = "Model 2 - Model 1",linewidth=0, levels=minR2:0.08:maxR2,showlabels = true,size=(1300,1000),xtickfont=18,ytickfont=18)

p = plot(p1,p2,p3,linewidth=0, showlabels = true,size=(1300,1000),xtickfont=18,ytickfont=18)
png(p, "output.png")


#p2=contourf(xg,yg,N[:,:,2],linewidth=0,levels=50,size=(1000,700),xtickfont=18,ytickfont=18)
#p1= contourf(xg,yg,M1_R2,linewidth=0.5,levels=20,size=(1200,700))
#p2= contourf(xg,yg,M2_R2,linewidth=0.5,levels=20,size=(1200,700))
#pd= contourf(xg,yg,M2_R2 - M1_R2,linewidth=0.5,levels=20,size=(1200,700))
#plot(p1,p2,pd,size=(1400,800))
